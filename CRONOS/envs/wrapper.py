...
class CronosWrapper:
    ...
    def __init__(self, args, unnorm_state, task_suite, device=None, task_scheduler=None):
        ...
        self.vla_type = getattr(args, "vla_type", "openvla")

    def _format_univla_action(self, raw_action: torch.Tensor) -> torch.Tensor:
        """Passes through the pre-decoded continuous 7D action from UniVLA's ActionDecoder.
        ActionDecoder outputs [world_vector(3), rotation_delta_euler(3), gripper(1)].
        CRONOS uses arm_pd_ee_target_delta_pose_align2_gripper_pd_joint_pos which accepts
        euler rotation delta directly — no euler→axis-angle conversion needed here.
        """
        return torch.cat([
            raw_action[:, :3].float(),                          # world_vector
            raw_action[:, 3:6].float(),                         # rotation_delta (euler)
            2.0 * (raw_action[:, 6:7].float() > 0.5) - 1.0,   # gripper binarized to {-1, +1}
    ...
    def step(self, raw_action):
        ...
        if self.vla_type == "univla":
            action = self._format_univla_action(raw_action)
        else:
