#!/bin/bash
# eval_1x1.sh - Run CRONOS evaluation on a 1-object / 1-receptacle scene
# (carrot + yellow plate) via PickPlaceNxM-v1 with N=1, M=1.

# OpenVLA evaluation
CUDA_VISIBLE_DEVICES=0 XLA_PYTHON_CLIENT_PREALLOCATE=false PYTORCH_CUDA_ALLOC_CONF=expandable_segments:True \
PYTHONPATH=$PYTHONPATH:$(pwd) \
python main.py \
    --name "CRONOS_Eval_1x1" \
    --seed 0 \
    --env_id "PickPlaceNxM-v1" --env_n 1 --env_m 1 \
    --obj1_index 1 --plate1_index 1 \
    --num_envs 16 \
