import os
os.environ.setdefault('TF_CPP_MIN_LOG_LEVEL', '3')   # suppress TF C++ INFO/WARNING/ERROR noise
...
class Args:
    ...
    # --- VLA type selector ---
    vla_type: str            = "openvla"  # "openvla" | "univla"
    univla_decoder_path: str = ""         # "" = auto-infer from vla_path/action_decoder.pt
    univla_window_size: int  = 10         # ActionDecoder temporal window; MUST match checkpoint
...
class CronosRunner:
    ...
    def __init__(self, args):
        ...
        elif args.vla_type == "univla":
            from simpler_env.policies.univla.univla_train import UniVLAPolicy
            self.policy = UniVLAPolicy(args, device_id=device_id_other, num_envs=args.num_envs)
        else:
            raise ValueError(f"Unknown vla_type: {args.vla_type!r}. Choose 'openvla' or 'univla'.")
        ...
        act_dim = 4 if args.vla_type == "univla" else 7
        self.buffer = CronosReplayBuffer(args, act_dim=act_dim)
    ...
    def _get_action(self, obs, instruct, deterministic=False):
        ...
        for i in range(0, total_batch, self.args.buffer_inferbatch):
            ...
            if self.args.vla_type == "univla":
                cont_actions.append(self.policy.get_cont_action())

        if self.args.vla_type == "univla":
            self._last_env_action = torch.cat(cont_actions, dim=0)
    ...
    def eval(self, iteration, task_idx, obj_set, envs_object, envs_recep, prefix="eval", reset=True):
        ...
        if self.args.vla_type == "univla":
            self.policy.reset_action_decoders()  # Clear temporal ensemble from prior task
        ...
        for _ in tqdm(range(self.args.segment_len), desc="eval", leave=False):
            ...
            env_input = self._last_env_action if self.args.vla_type == "univla" else action
            obs, reward, truncated, env_info = self.env.step(env_input)
    ...
    def run_rollout(self, ppo_log_path=None, episode=0, episode_base_steps=0, episode_base_resets=0):
        ...
        for step_idx in tqdm(range(self.args.episode_len), desc="Rollout", leave=False):
            ...
            # 2. Environment Step
            env_input = self._last_env_action if self.args.vla_type == "univla" else action
            next_obs, reward, truncated, info = self.env.step(env_input)
            ...
            if (step_idx + 1) % self.args.task_len == 0:
                ...
                if self.args.vla_type == "univla":
                    self.policy.reset_action_decoders()
