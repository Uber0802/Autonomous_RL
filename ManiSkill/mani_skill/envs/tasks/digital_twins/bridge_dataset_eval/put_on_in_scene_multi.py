...
class PutOnPlateInScene25(BaseEnv):
    ...
    def __init__(self, **kwargs):
        ...
        self._instruction_toggle = False
        self.instruction = ""
    ...
    def _load_scene(self, options: dict):
        ...
        # plate
        self.objs_plate: dict[str, list[Actor]] = {}
        ...
        for idx, name in enumerate(self.model_db_plate):
            ...
            actor1 = self._build_actor_helper(name + "_1", model_path, density, scale, Pose.create_from_pq(torch.tensor([2.0, 0.3 * idx, 1.0])))
            actor2 = self._build_actor_helper(name + "_2", model_path, density, scale, Pose.create_from_pq(torch.tensor([2.5, 0.3 * idx, 1.0])))
            ...
            self.objs_plate[name] = [actor1, actor2]
    ...
    def _initialize_episode(self, env_idx: torch.Tensor, options: dict):
        ...
        select_plate = [self.plate_names[idx] for idx in self.select_plate_ids_1]
        # plate_actor = [self.objs_plate[n] for n in select_plate]
        select_plate_2 = [self.plate_names[idx] for idx in self.select_plate_ids_2]
        # plate_actor_2 = [self.objs_plate[n] for n in select_plate_2]

        plate_actor = [self.objs_plate[n][0] for n in select_plate]  # plate 1
        plate_actor_2 = [self.objs_plate[n][1] for n in select_plate_2]  # plate 2
        ...
        for idx, name in enumerate(self.model_db_plate):
            is_select_1 = self.select_plate_ids_1 == idx
            is_select_2 = self.select_plate_ids_2 == idx
            p_reset = torch.tensor([2.0, 0.3 * idx, 1.0], device=self.device).reshape(1, -1).repeat(b, 1)

            p_select_1 = xyz_configs[self.select_pos_ids, 1].reshape(b, 3)
            p_select_2 = xyz_configs[self.select_pos_ids, 2].reshape(b, 3)

            p_1 = torch.where(is_select_1.unsqueeze(1).repeat(1, 3), p_select_1, p_reset)
            p_2 = torch.where(is_select_2.unsqueeze(1).repeat(1, 3), p_select_2, p_reset)

            q_reset = torch.tensor([0, 0, 0, 1], device=self.device).reshape(1, -1).repeat(b, 1)
            q_select_1 = quat_configs[self.select_quat_ids, 1].reshape(b, 4)
            q_select_2 = quat_configs[self.select_quat_ids, 2].reshape(b, 4)

            q_1 = torch.where(is_select_1.unsqueeze(1).repeat(1, 4), q_select_1, q_reset)
            q_2 = torch.where(is_select_2.unsqueeze(1).repeat(1, 4), q_select_2, q_reset)

            self.objs_plate[name][0].set_pose(Pose.create_from_pq(p=p_1, q=q_1))  # plate 1
            self.objs_plate[name][1].set_pose(Pose.create_from_pq(p=p_2, q=q_2))  # plate 2
    ...
    def evaluate(self, success_require_src_completely_on_target=True):
        ...
        carrot_actor = [self.objs_carrot[n] for n in select_carrot]
        # plate_actor = [self.objs_plate[n] for n in select_plate]
        select_plate_1 = [self.plate_names[idx] for idx in self.select_plate_ids_1]
        select_plate_2 = [self.plate_names[idx] for idx in self.select_plate_ids_2]
        plate_actor_1 = [self.objs_plate[n][0] for n in select_plate_1]  # plate1 actor
        plate_actor_2 = [self.objs_plate[n][1] for n in select_plate_2]  # plate2 actor

        


        plate_p1 = torch.stack([a.pose.p[idx] for idx, a in enumerate(plate_actor_1)])  # [b, 3]
        plate_q1 = torch.stack([a.pose.q[idx] for idx, a in enumerate(plate_actor_1)])  # [b, 4]
        plate_p2 = torch.stack([a.pose.p[idx] for idx, a in enumerate(plate_actor_2)])  # [b, 3]
        plate_q2 = torch.stack([a.pose.q[idx] for idx, a in enumerate(plate_actor_2)])  # [b, 4]
        ...
        offset1 = pos_src - plate_p1
        offset2 = pos_src - plate_p2

        xy_flag1 = (
            torch.linalg.norm(offset1[:, :2], dim=1)
            <= tgt_obj_half_length_bbox.max(dim=1).values + xy_flag_required_offset
        )
        z_flag1 = (offset1[:, 2] > 0) & (
            offset1[:, 2] - tgt_obj_half_length_bbox[:, 2] - src_obj_half_length_bbox[:, 2]
            <= z_flag_required_offset
        )
        src_on_target1 = xy_flag1 & z_flag1
        ...
        # src_on_target = src_on_target1 | src_on_target2  # 任一個成功就成功
        ...
        if success_require_src_completely_on_target:
            net_forces1 = torch.zeros((b,), dtype=torch.float32, device=self.device)
            net_forces2 = torch.zeros((b,), dtype=torch.float32, device=self.device)
            for idx in range(self.num_envs):
                f1 = self.scene.get_pairwise_contact_forces(
                    self.objs_carrot[select_carrot[idx]],
                    self.objs_plate[select_plate_1[idx]][0],
                )[idx]
                f2 = self.scene.get_pairwise_contact_forces(
                    self.objs_carrot[select_carrot[idx]],
                    self.objs_plate[select_plate_2[idx]][1],
                )[idx]
                net_forces1[idx] = torch.linalg.norm(f1)
                net_forces2[idx] = torch.linalg.norm(f2)

        if hasattr(self, "_instruction_toggle") and self._instruction_toggle:
            # print("1 self._instruction_toggle : ", self._instruction_toggle, "plate 1 : ", select_plate_1, "plate 2 : ", select_plate_2, self.instruction)
            src_on_target = src_on_target1
            if success_require_src_completely_on_target:
                src_on_target = src_on_target & (net_forces1 > netforce_flag_required_offset)
        else:
            # print("2 self._instruction_toggle : ", self._instruction_toggle, "plate 1 : ", select_plate_1, "plate 2 : ", select_plate_2, self.instruction)
            src_on_target = src_on_target2
            if success_require_src_completely_on_target:
                src_on_target = src_on_target & (net_forces2 > netforce_flag_required_offset)
        ...
        gripper_plate_dist_1 = torch.linalg.norm(gripper_p - plate_p1, dim=1)  # [b, 3]
        gripper_plate_dist_2 = torch.linalg.norm(gripper_p - plate_p2, dim=1)  # [b, 3]
        carrot_plate_dist_1 = torch.linalg.norm(carrot_p - plate_p1, dim=1)  # [b, 3]
        carrot_plate_dist_2 = torch.linalg.norm(carrot_p - plate_p2, dim=1)  # [b, 3]
        ...
        self.episode_stats["gripper_plate_dist_1"] = gripper_plate_dist_1
        self.episode_stats["gripper_plate_dist_2"] = gripper_plate_dist_2
        self.episode_stats["carrot_plate_dist_1"] = carrot_plate_dist_1
        self.episode_stats["carrot_plate_dist_2"] = carrot_plate_dist_2
        ...
        self.extra_stats["extra_pos_plate_1"] = plate_p1
        self.extra_stats["extra_pos_plate_2"] = plate_p2
        self.extra_stats["extra_q_plate_1"] = plate_q1
        self.extra_stats["extra_q_plate_2"] = plate_q2
    ...
    #         plate_name_1 = self.model_db_plate[select_plate_1[idx]]["name"]
    #         plate_name_2 = self.model_db_plate[select_plate_2[idx]]["name"]
    #         instruct.append(f"put {carrot_name} on {plate_name_1} or {plate_name_2}")

    #     return instruct

    def get_language_instruction(self):
        # 初始化 toggle 用於交替 plate_name_1 / plate_name_2
        if not hasattr(self, "_instruction_toggle"):
            self._instruction_toggle = False  # 初始為 False 代表先用 plate_name_1
        ...
        for idx in range(self.num_envs):
            carrot_name = self.model_db_carrot[select_carrot[idx]]["name"]
            if self._instruction_toggle:
                plate_name = self.model_db_plate[select_plate_2[idx]]["name"]
            else:
                plate_name = self.model_db_plate[select_plate_1[idx]]["name"]
            instruct.append(f"put {carrot_name} on {plate_name}")
        self.instruction = instruct

        # 反轉 toggle 為下次 call 做準備
        self._instruction_toggle = not self._instruction_toggle
...
class PutOnPlateInScene25MainV3(PutOnPlateInScene25):
    ...
    def _prep_init(self):
        ...
        keys = list(self.model_db_plate.keys())
        selected_keys = [keys[0], keys[17]]
        self.model_db_plate = {k: self.model_db_plate[k] for k in selected_keys}
    ...
    def _initialize_episode_pre(self, env_idx: torch.Tensor, options: dict):
        ...
        self.select_plate_ids_1 = (episode_id // (lo * l1 * l2)) % lp
        self.select_plate_ids_2 = (self.select_plate_ids_1 + 1) % lp
    ...
    def _generate_init_pose(self):
        ...
        for i, grid_pos_1 in enumerate(grid_pos):         # carrot
            for j, grid_pos_2 in enumerate(grid_pos):     # plate 1
                for k, grid_pos_3 in enumerate(grid_pos): # plate 2
                    if i != j and i != k and j != k:
                        if (
                            np.linalg.norm(grid_pos_2 - grid_pos_1) > 0.07 and
                            np.linalg.norm(grid_pos_3 - grid_pos_1) > 0.07 and
                            np.linalg.norm(grid_pos_3 - grid_pos_2) > 0.1
                        ):
                            xyz_configs.append(
                                np.array([
                                    np.append(grid_pos_1, 0.95),      # carrot
                                    np.append(grid_pos_2, 0.95),  # plate 1
                                    np.append(grid_pos_3, 0.95),  # plate 2
                                ])
                            )
        ...
        quat_configs = np.stack([
            np.array([
                ...
                euler2quat(0, 0, 0.0),           # plate1
                euler2quat(0, 0, 0.0),           # plate2
            ]),
            np.array([
                euler2quat(0, 0, np.pi / 4),
                euler2quat(0, 0, np.pi / 4),
                euler2quat(0, 0, np.pi / 4),
            ]),
            np.array([
