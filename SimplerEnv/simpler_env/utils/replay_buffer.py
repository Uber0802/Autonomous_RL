...
class SeparatedReplayBuffer(object):
    def __init__(self, all_args, obs_dim, act_dim, action_dtype=np.int32, logprob_dim=None):
        ...
        # logprob_dim defaults to act_dim (OpenVLA: per-token logprobs)
        # For CogACT: logprob_dim=1 (single scalar logprob per action)
        ...
        self.actions = np.zeros((self.ep_len, self.num_env, act_dim), dtype=action_dtype)
...
class PreallocReplayBuffer(SeparatedReplayBuffer):
    def __init__(self, all_args, obs_dim, act_dim, action_dtype=np.int32, logprob_dim=None):
        ...
        self.actions = create_memmap('actions.dat', (self.ep_len, max_num_envs, act_dim), action_dtype)
...
class FIFOReplayBuffer(SeparatedReplayBuffer):
    def __init__(self, all_args, obs_dim, act_dim, max_num_envs, action_dtype=np.int32, logprob_dim=None):
        self.ep_len = all_args.episode_len
        self.gamma = all_args.buffer_gamma
        self.gae_lambda = all_args.buffer_lambda
        self.buffer_minibatch = all_args.buffer_minibatch
