    ...
    def feed_forward_generator(self):
        ...
        for indices in sampler:
            ...
            yield (obs_batch, instruct_batch, actions_batch, value_preds_batch, return_batch, masks_batch,
                   old_action_logits_batch, adv_targ, reward_batch, next_obs_batch)
...
class ResSeparatedReplayBuffer(object):
    def __init__(self, all_args, obs_dim, act_dim):
        self.ep_len = all_args.episode_len
        self.num_env = all_args.num_envs
        self.gamma = all_args.buffer_gamma
    ...
    def cat_buffer(self, buffer: 'ResSeparatedReplayBuffer'):
        assert self.ep_len == buffer.ep_len, "Episode lengths must match"
        assert self.obs.shape[0] == buffer.obs.shape[0], "Obs time dimension mismatch"
        assert self.obs.shape[2:] == buffer.obs.shape[2:], "Obs shape mismatch"
        ...
        self.masks = np.concatenate([self.masks, buffer.masks], axis=1)
        self.advantages = np.concatenate([self.advantages, buffer.advantages], axis=1)
        self.hidden = np.concatenate([self.hidden, buffer.hidden], axis=1)
        self.base_action = np.concatenate([self.base_action, buffer.base_action], axis=1)
        self.action_untok = np.concatenate([self.action_untok, buffer.action_untok], axis=1)
    ...
    def feed_forward_generator(self):
        ...
        action_logits = self.action_log_probs.reshape(-1, self.action_log_probs.shape[-1])
        advantages = self.advantages.reshape(-1, 1)
        hiddens = self.hidden[:-1].reshape(-1, self.hidden.shape[-1])
        base_actions = self.base_action.reshape(-1, self.actions.shape[-1])
        actions_untok = self.action_untok.reshape(-1, self.actions.shape[-1])
        ...
        for indices in sampler:
            ...
            adv_targ = advantages[indices]
            reward_batch = rewards[indices]
            hidden_batch = hiddens[indices]
            base_actions_batch = base_actions[indices]
            actions_untok_batch = actions_untok[indices]
            ...
            yield (obs_batch, instruct_batch, actions_batch, value_preds_batch, return_batch, masks_batch,
                   old_action_logits_batch, adv_targ, reward_batch, hidden_batch, base_actions_batch, actions_untok_batch)
