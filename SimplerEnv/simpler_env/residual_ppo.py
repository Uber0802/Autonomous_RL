...
from simpler_env.utils.replay_buffer import SeparatedReplayBuffer, ResSeparatedReplayBuffer
...
class Args:
    ...
    name: str = "PPO-test"
    log: str = "/workspace/Autonomous_RL/log.txt"
...
class Runner:
    def __init__(self, all_args: Args):
        ...
        # alg_name
        assert self.args.alg_name in ["sac", "ppo", "grpo"]
    ...
    @torch.no_grad()
    def _get_action_costmap(self, envs, obs, costmap_handler, subtask_id, viz_writers, hidden_tensor, deterministic=False):
        total_batch = obs["image"].shape[0]
        topk = 10 
        ...
        for i in range(0, total_batch, self.args.buffer_inferbatch):
            ...
            for j in range(self.args.buffer_inferbatch):
                ...
                base_last = a_base[:, 6:].expand(delta_a.shape[0], -1)  
                a_candidates = torch.cat([a_base[:, :6] + delta_a, base_last], dim=1)
                a_candidates = torch.clamp(a_candidates, min=action_low, max=action_high)
                
    
                normalized_actions = 2 * (a_candidates - action_low) / (action_high - action_low) - 1.0
                ...
                for k in range(len(a_candidates)):
                    cand = a_candidates[k].unsqueeze(0)
                    ...
                    final_candidates.append((cost, grid_idx, token_ids[k], a_candidates[k]))

                
                topk_candidates = sorted(final_candidates, key=lambda x: x[0])[:topk]
                ...
                selected_cost, _, selected_action, selected_action_tok = selected
        ...
        base_actions = torch.cat(base_actions, dim=0).to(self.device)
        final_actions_untok = torch.cat(final_actions_tok, dim=0).to(device=self.device)



        return final_values, final_actions, final_logprobs, final_costs, base_actions, final_actions_untok
    ...
    def collect_costmap(self, envs, costmap_handler, subtask_id, viz_writers, hidden_tensor):
        self.policy.prep_rollout()

        obs_image = self.buffer.obs[self.buffer.step]
        obs_image = torch.tensor(obs_image).to(self.device)
        obs = dict(image=obs_image, task_description=self.buffer.instruction)
        value, action, logprob, cost, base_action, action_untok = self._get_action_costmap(envs=envs, obs=obs, costmap_handler=costmap_handler, subtask_id=subtask_id, viz_writers=viz_writers, hidden_tensor=hidden_tensor)
        # with open("/workspace/AutoRL_SD/log.txt", "a") as f:
        #     f.write(f"logprob : {logprob}\n")

        return value, action, logprob, cost, base_action, action_untok
    ...
    def eval_old(self, obj_set: str, object: list[str], receptacle: list[str], episode=-1) -> dict:
        ...
        for i in range(self.args.num_envs):
            viz_path = f"eval_video/{episode}/costmap_vis_{i:04d}.mp4"
        ...
        print("Evaluating:", instruction[0], instruction[8], instruction[16], instruction[24])
    ...
    def eval(
        ...
        for _ in range(self.args.episode_len):
            ...
            hidden_tensor = torch.cat(all_hidden, dim=0) 
            _, action, _, _, _ = self._get_action(obs, hidden_tensor, deterministic=True)
    ...
    def run(self):
        ...
        for episode in range(max_episodes):
            ...
            for step_idx in tqdm(range(self.args.training_len), desc="rollout"):
                ...
                # if episode <= 50:
                value, action, logprob, best_cost, base_action, action_untok = self.collect_costmap(envs=self.env, costmap_handler=costmap_handler, subtask_id=subtask_id, viz_writers=viz_writers, hidden_tensor=hidden_tensor)
                # import ipdb; ipdb.set_trace()
                # else:
            ...
            if episode % self.args.interval_eval == self.args.interval_eval - 1 or episode == max_episodes - 1:
                ...
                for i in range(4):
                    ...
                    objects.extend([obj] * group_size)
                    receptacles.extend([recep] * group_size)
                sval_stats = self.eval_old("train", objects, receptacles, episode)
